...
from model import BaselineModel as l3_dense
...
if args.model_type =='audio':
    ...
    path_input = os.path.join(args.features_path, 'audio_features_data/tt.hdf5')

    global_mean_std_path = os.path.join(args.features_path, 'audio_features_data/global_mean_std.npz')
...
if args.model_type == 'video':
    path_input = os.path.join(args.features_path, 'video_features_data/tt.hdf5')
    global_mean_std_path = os.path.join(args.features_path, 'video_features_data/global_mean_std.npz')
    mean_std = np.load(global_mean_std_path)
    mean = mean_std['global_mean']
    std = mean_std['global_std']
    model = l3_dense(512,10)
...
if args.model_type == 'audio_video':
    ...
    global_mean_std_path_audio = os.path.join(args.features_path, 'audio_features_data/global_mean_std.npz')
    ...
    global_mean_std_path_video = os.path.join(args.features_path, 'video_features_data/global_mean_std.npz')
...
# TODO: change to load actual model weights
...
if args.model_type=='video':
    ...
    df.to_csv('audio_video_output_v1.csv',sep='\t',index= False)
#embed()
##########evaluation##########
...
plt.savefig('cm'+args.model_type+'.png')
